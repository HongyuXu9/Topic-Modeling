...
def get_topic_words(token_lists, labels, k=None):
    ...
    for counts in word_counts:
        total_count = sum(count for word, count in counts)  # somma di tutti i conteggi
        if total_count > 0:  # evita divisione per zero
            normalized_counts = [(word, count / total_count) for word, count in counts]
        else:
            normalized_counts = counts  # Se non ci sono conteggi, mantieni i conteggi originali
        word_counts_normalized.append(normalized_counts)

    # get topics with normalized weights
    topics = list(map(lambda x: sorted(x, key=lambda x: x[1], reverse=True)[:10], word_counts_normalized))
...
def plot_topics(model, token_lists, topic):
    print(f'Plotting topics for topic {topic} ...')
    lbs = model.cluster_model.labels_
    
    # Filtra le liste in base all'argomento
    filtered_tokens = [token_list for i, token_list in enumerate(token_lists) if lbs[i] == topic and isinstance(token_list, list)]
    
    # Controlla se ci sono token filtrati
    if filtered_tokens:
        # Flatten the list of token lists into a single list
        all_tokens = [token for token_list in filtered_tokens for token in token_list]
        
        # Crea un grafico a barre dei token
        token_counts = {token: all_tokens.count(token) for token in set(all_tokens)}
        sorted_tokens = sorted(token_counts.items(), key=lambda x: x[1], reverse=True)[:10]  # Top 10 tokens
        tokens, counts = zip(*sorted_tokens)
        
        # Plot dei token
        plt.figure(figsize=(10, 6))
        plt.bar(tokens, counts, color='skyblue')
        plt.title(f'Top Tokens for Topic {topic}')
        plt.xlabel('Tokens')
        plt.ylabel('Counts')
...
def preprocess(docs):
    ...
    idx_in = []  # index of documents processed
...
class Topic_Model:
    ...
    def predict(self, sentences):
        """
        Predict the topic for each document based on the fitted model.
        :param sentences: List of sentences (documents)
        :return: List of predicted topics for each document
        """
        if self.method != 'LDA_BERT':
            raise Exception('Predict is only available for LDA_BERT method.')
    
        # Get topic distribution for each document
        predictions = []
        for doc in self.corpus:
            topic_probs = self.ldamodel.get_document_topics(doc)
            # Get the topic with the highest probability
            if topic_probs:
                predictions.append(max(topic_probs, key=lambda x: x[1])[0])
            else:
                predictions.append(-1)  # No topic assigned
